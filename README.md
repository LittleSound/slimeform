...
So I started to experiment a solution to accomplish this kind of feature, and naming it with SlimeForm, which means this utilities would try it best to fit in the forms just like the slime does 💙.

SlimeForm is a form state management and validator which has **dependency free**, **internal validation rules free** features. By binding all native or custom components through `v-model`, SlimeForm is able to manage and validate values reactively.
